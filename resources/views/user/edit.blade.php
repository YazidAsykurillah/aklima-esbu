...
<div class="card">
    ...
    <div class="card-body">
       <form method="POST" action="{{ url('user/'.$user->id.'') }}" aria-label="{{ __('Store user') }}">
        @csrf
        {{ method_field('PATCH') }}
            <div class="form-group row">
                <label for="name" class="col-sm-2 col-form-label text-md-right">{{ __('Nama') }}</label>
                <div class="col-md-6">
                    <input id="name" type="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') ? old('name') : $user->name }}" required>
                    @if ($errors->has('name'))
                ...
                <div class="col-md-6">
                    <select class="form-control" id="provinsi_id" name="provinsi_id" placeholder="Pilih Provinsi">
                        ...
                        @foreach($provinsi_options as $provinsi)
                            <option value="{{ $provinsi->uid_provinsi }}" {{ $user->provinsi_id == $provinsi->uid_provinsi ? 'selected' : ''}} >{{$provinsi->nama_provinsi }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row mb-0">
                <div class="col-md-8 offset-md-4">
                    ...
                    <a class="btn btn-link" href="{{ url('user/'.$user->id.'') }}">
                        {{ __('Cancel') }}
                    </a>
                </div>
            </div>
